...
public func zip<R0: ReducibleType, R1: ReducibleType>(r0: R0, r1: R1) -> Stream<(R0.Element, R1.Element)> {
	let streams = (Stream(r0), Stream(r1))
	switch (streams.0, streams.1) {
	case let (.Cons(x, xs), .Cons(y, ys)):
		return .cons((x.value, y.value), zip(xs.value, ys.value))
	default:
		return nil
	}
}
...
public func zip<R0: ReducibleType, R1: ReducibleType, R2: ReducibleType>(r0: R0, r1: R1, r2: R2) -> Stream<(R0.Element, R1.Element, R2.Element)> {
	let streams = (Stream(r0), Stream(r1), Stream(r2))
	switch (streams.0, streams.1, streams.2) {
	case let (.Cons(x, xs), .Cons(y, ys), .Cons(z, zs)):
		return .cons((x.value, y.value, z.value), zip(xs.value, ys.value, zs.value))
	default:
		return nil
	}
}
